...
Analyse-dashboard op `GET /admin/analyse` (zelfde basic-auth): bundelt de regio-recaps tot twee visualisaties + een AI-verslag (`POST /admin/verslag`, model `claude-opus-4-8`; zonder `ANTHROPIC_API_KEY` een getemplate samenvatting). Het verslag wordt als Markdown gerenderd (kleine inline renderer in `analyse.html`, geen externe lib i.v.m. CSP), met een lezen/bewerken-toggle; lengte volgt de inhoud (geen 1-A4-limiet). Regio-map bewerkbaar via `POST /admin/regios`, opgeslagen als `<RECAP_DIR>/regios.json` (geseed met HRQT/WTEL/PUXD/MDRH). Aggregatielogica staat los in `app/analyse-lib.mjs` (unit-getest via `tests/analyse-lib.spec.mjs`).
